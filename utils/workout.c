...
void get_workouts_by_exercise_name(const char *name, WorkoutLog *workouts)
{
    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        char workout_id[20] = "", exercise[100] = "", sets[20] = "", reps[20] = "",
             weight[50] = "", time[20] = "", distance[20], date_field[20] = "", notes[200] = "";
...
{
    WorkoutLog workouts[100] = {0};
    get_workouts_by_exercise_name(name, workouts);
    int length = get_workout_array_length(workouts, 100);
    if (length == 0)
    {
        printf(ANSI_COLOR_YELLOW "No workouts found for exercise '%s'.\n" ANSI_COLOR_RESET, name);
        return 0;
    }
    print_workouts(workouts);
    return 0;
